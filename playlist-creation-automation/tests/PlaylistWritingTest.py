...
class PlaylistWritingTest(unittest.TestCase):
    ...
    def test_get_playlist_file_path(self):
        ...
        self.assertEqual(
            playlist_file_path, expected,
            "Expected '{0}' to be equal to '{1}'.".format(playlist_file_path, expected)
        )
    ...
    def test_get_file_absolute_path_from_playlist_element(self):
        ...
        self.assertEqual(
            file_protocol_element_path, expected,
            "Expected '{0}' to be equal to '{1}'.".format(file_protocol_element_path, expected)
        )
    ...
    def test_write_playlist_start(self):
        ...
        with open(file_path, mode='w') as f:
            ...
            for expected_elt in ['<?xml', '<playlist xmlns="', '<title>', '<trackList>']:
                self.assertTrue(
                    self.assertFileContains(f, expected_elt),
                    "'{0}' does not contain '{1}'".format(file_path, expected_elt)
                )
